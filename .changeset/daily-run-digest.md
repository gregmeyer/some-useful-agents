...
agent — a short output snippet for successes, and a link to the existing
run-failure thread for failures (never restating the error). Empty days are
skipped; exactly one thread per day (restart-safe, catches up the previous day
after downtime). It's the first real `cadence` producer, runs in-process in the
