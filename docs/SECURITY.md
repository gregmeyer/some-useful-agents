...
|------|--------|-------------------|
| `community` | Agents in `agents/community/`, installed from third parties | Hostile-by-assumption. Minimal env allowlist; shell downstream blocked; llm-prompt output wrapped. |
...
### Orphan process reaper (v0.21.0)

**What it defends against**: a dashboard restart (`daemon restart`, crash, OOM) that lands mid-run leaves the spawned LLM CLI orphaned. Before the fix the child got reparented to launchd / init, kept calling Anthropic for the full request, and burned tokens until it completed naturally; the `runs` row sat at `status='running'` indefinitely because the in-memory `setTimeout(SIGTERM)` armed in `node-spawner` died with the parent process. We reproduced a 13-minute hang on a normally-20-second `layout-planner` run.

**Mechanism**, two parts working together:

1. **State-machine close on boot.** `startDashboardServer` calls `reapOrphanedRuns(runStore)` after `RunStore` is constructed. The function pulls every run where `status IN ('running','pending')` — by definition orphans because the only process that could be executing them is the dashboard, which just started fresh. Each run + every still-`running` `node_executions` row is transitioned to `failed` with `errorCategory='abandoned'`, so dashboards stop polling, notify logic doesn't fire forever, and the audit trail explains the gap.
2. **Actual process kill, defended against PID reuse.** When `spawn()` returns, the executor writes the child's `pid` and `Date.now()` to the in-flight `node_executions` row as `childPid` and `childStartedAtMs`. The reaper, for every row that carries both fields, runs `ps -p <pid> -o etime=`, parses the `[[DD-]HH:]MM:SS` output, and compares against `(now - childStartedAtMs) / 1000`. If they match within `max(5s, 10% of expected)`, the PID is still ours and we `process.kill(pid, 'SIGKILL')`. If they've drifted (the original child died and the kernel reassigned the PID to something unrelated), we skip the kill — orphan was already gone, and we won't murder an unrelated process.

**Cancel path also escalates.** Independently of the boot reaper, the spawnProcess abort handler now mirrors the timeout path: SIGTERM, then SIGKILL after 5s if the child is still alive. A claude/codex CLI stuck in a slow HTTP read can no longer ignore SIGTERM indefinitely. The cancel route's fallback (when `activeRuns` is empty because the dashboard restarted between kickoff and cancel) also walks every `running`/`pending` node execution and finalizes them — the old fallback only touched the `runs` row.

**Operator awareness.** Boot logs surface the counts: `[orphan-reaper] Finalized 1 run(s) and 1 node execution(s) (killed 1 orphaned process(es)) left in non-terminal state by a prior dashboard exit. Run ids: <id>...`. The new `'abandoned'` category is exposed in the run-detail UI and is available in retry-policy `categories:` filters if you want to auto-retry abandoned runs on next boot.

**Limits.** The reaper is an on-boot pass; it does not run continuously. An orphan whose parent dashboard never restarts stays orphaned. The `ps` cross-check is best-effort — if `ps` is unavailable (sandboxed environments) or returns unparseable output, we skip the kill rather than risk a false positive. Pair the reaper with per-node `timeout:` and agent-level `timeoutSec:` (see [agents.md § Timeouts](agents.md)) for in-flight protection.
...
- **Prompt injection from llm-prompt upstream agents.** We only wrap values from `community` agents. If you write a `local` llm-prompt agent that can be manipulated by a user into producing prompt-injection payloads, and a second `local` agent reads its output, that output flows through unwrapped. Don't chain agents whose inputs you don't control.
...
2. **Audit community agents before installing.** Run `sua agent audit <name>` to print the resolved YAML, read the `command:`, `prompt:`, and `envAllowlist:` fields, then opt in with `--allow-untrusted-shell <name>` per invocation. Prefer `type: llm-prompt` over `type: shell` when possible.
