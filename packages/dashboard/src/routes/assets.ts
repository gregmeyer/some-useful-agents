...
/**
 * Wiring editor for the agent-detail DAG. Kept as its own asset rather than
 * grown onto GRAPH_RENDER_JS: it only loads where editing is possible, and
...
(function () {
  ...
  function connect(sourceId, targetId) {
    var c = cy();
    if (sourceId === targetId) { flash('A node cannot depend on itself.'); return; }
...
assetsRouter.get('/assets/graph-edit.js', (_req: Request, res: Response) => {
  ...
  res.type('application/javascript').send(GRAPH_EDIT_JS);
});
